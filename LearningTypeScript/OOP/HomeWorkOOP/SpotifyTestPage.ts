...
class SpotifyTestPage{
    ...
    subscriptionType = 'Family Premium Plan'
    skipLimit = 728
    defaultAudioQuality = 420
    ...
    monthlyListeningTime (dailyMinutesListened: number, daysInTheMonth: number) : number{
        let totalMinutes: number = dailyMinutesListened * daysInTheMonth
        return totalMinutes
    }

    fileFormat(desiredFileFormat : string) : string []{
        let songFileNames : string [] = ["song1.mp3", "track2.flac", "music3.mp3", "beat4.wav", "tune5.mp3", "sound6.flac"]
        let fileFormatResult : string [] = []

        for ( let i = 0; i < songFileNames.length; i++){
            if(songFileNames[i].endsWith(".mp3")){
                fileFormatResult.push(songFileNames[i])
            }
        }
...
let testSpotifyPage = new SpotifyTestPage()

console.log(testSpotifyPage.subscriptionType)
console.log(testSpotifyPage.skipLimit)
console.log(testSpotifyPage.defaultAudioQuality)

testSpotifyPage.startMusic()
console.log(testSpotifyPage.monthlyListeningTime(85, 31))
console.log(testSpotifyPage.fileFormat(".mp3"))
